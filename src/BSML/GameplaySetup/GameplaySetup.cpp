...
namespace BSML {
    safe_ptr<GameplaySetup*> GameplaySetup::instance;
    ...
    void GameplaySetup::Setup() {
        ...
        parse_and_construct(Assets::GameplayMenu::GameplaySetup, t, this);
    ...
    void GameplaySetup::GameplaySetupDidActivate(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling) {
        ...
        if (fc.try_cast<GlobalNamespace::CampaignFlowCoordinator>()) {
            menuType = MenuType::Campaign;
        } else if (fc.try_cast<GlobalNamespace::SinglePlayerLevelSelectionFlowCoordinator>()) {
            menuType = MenuType::Solo;
        } else if (fc.try_cast<GlobalNamespace::GameServerLobbyFlowCoordinator>()) {
    ...
    GameplaySetupCell* GameplaySetup::GetCell() {
        INFO("Getting Cell");
        auto cell = modsList->tableView->DequeueReusableCellForIdentifier(reuseIdentifier).try_cast<GameplaySetupCell>();
        ...
        if (!cell || !cell->m_CachedPtr.m_value) {
            ...
            parse_and_construct(Assets::GameplayMenu::GameplaySetupCell, cell->get_transform(), cell);
        }
    ...
    ListW<System::Object*> GameplaySetup::get_menus() {
        if (!_menus) {
            _menus = ListW<System::Object*>::New();
        }
        return _menus;
    }
    ...
    ListW<UnityEngine::Transform*> GameplaySetup::get_vanillaItems() {
        if (!_vanillaItems) {
            _vanillaItems = ListW<UnityEngine::Transform*>::New();
        }
